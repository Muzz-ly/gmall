...
public class GmallCacheAspect {
    ...
    public Object around(ProceedingJoinPoint joinPoint) throws Throwable {
        ...
        String jason2 = this.redisTemplate.opsForValue().get(prefix + param);
        //判断缓存中的数据是否为空
        if (StringUtils.isNotBlank(jason2)){
            return JSON.parseObject(jason2,returnType);
        }
        //执行目标方法
        ...
        this.redisTemplate.opsForValue().set(
                prefix + param,JSON.toJSONString(result),timeout + new Random().nextInt(random), TimeUnit.MINUTES);
        rLock.unlock();
        return result;
    }
}
